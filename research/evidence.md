...
## Added 2026-09-19 from two surveys the speaker supplied (research/2512.13564v2.pdf, research/2507.21046v4.pdf)
Page-referenced quotes for every claim below are in research/notes-two-surveys.md. Slides touched: 4 (heterogeneous models per agent), 6 (lossy summarization, drift, hierarchical folding), 7 (constructive human recall vs verbatim agent memory), 8 (goal drift; bottleneck is state maintenance), 9 notes (formal counterparts of the axes), 10 (learned orchestration: ADAS, AFlow, Puppeteer, cost caveat), 11 (isolation trade-off; write contention), 13 (consensus reliance), receipts (audit trail, version-controlled safe states), close (sandboxing and approval gates on the compliance checklist). The fact-checker should verify against the PDFs in research/, not the web.
